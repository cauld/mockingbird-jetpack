 ...
 */
    ...
    define("YAID", "YOUR ID HERE"); //Your Yahoo! Web Services App ID (https://developer.yahoo.com/wsregapp/)
    ...
    if ($cError == 0) {
        ...
        if ($cError == 0) {
            ...
            if ($yteResult) {
                ...
                //For the demo site we don't want twitter hogging the tweet list so kill it
                for($i=0; $i<count($keyTerms); $i++) {
                    if ($keyTerms[$i] == 'twitter' || $keyTerms[$i] == 'Twitter') {
                       unset($keyTerms[$i]);
                    }
                }
                $keyTerms= array_values($keyTerms);
                //End demo tweak
